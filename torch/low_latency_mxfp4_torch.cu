...
torch::Tensor requantize_deepep_fp8_compact_out(
    torch::Tensor input, torch::Tensor input_group_scales,
  ...
  TORCH_CHECK(input_group_scales.is_cuda(),
              "input_group_scales must be a CUDA tensor");
  TORCH_CHECK(input.element_size() == 1 && output.element_size() == 1,
              "input and output must use one-byte FP8 storage");
  TORCH_CHECK(input_group_scales.scalar_type() == torch::kFloat32,
              "input_group_scales must be float32");
  ...
  TORCH_CHECK(input.dim() == 3 && input_group_scales.dim() == 3,
              "DeepEP input and group scales must be 3D");
  ...
  TORCH_CHECK(input_group_scales.size(0) == experts &&
                  input_group_scales.size(1) == capacity &&
                  input_group_scales.size(2) == hidden_size / 128,
              "DeepEP group scales must have shape [E, capacity, H/128]");
  ...
  mga::launch_low_latency_mxfp4_fp8_requantize_deepep_compact(
      reinterpret_cast<const __nv_fp8_e4m3*>(input.data_ptr()),
      input_group_scales.data_ptr<float>(), token_counts.data_ptr<int32_t>(),
      compact_offsets.data_ptr<int32_t>(), experts, capacity, hidden_size,
      input_group_scales.stride(0), input_group_scales.stride(1),
      input_group_scales.stride(2),
...
torch::Tensor grouped_gemm_out_impl(
  ...
  launch.build_device_schedule = !tile_schedule_is_precomputed;
  launch.outs =
...
torch::Tensor deepep_fp8_moe_out(
    torch::Tensor recv_q, torch::Tensor recv_group_scales,
    torch::Tensor w13, torch::Tensor w13_exp_offsets,
    torch::Tensor w13_residual, torch::Tensor w2,
    torch::Tensor w2_exp_offsets, torch::Tensor w2_residual,
    torch::Tensor masked_m, torch::Tensor padded_offsets,
  ...
  requantize_deepep_fp8_compact_out(
      recv_q, recv_group_scales, masked_m, compact_offsets, q1, q1_scales);
  grouped_gemm_out_impl(
